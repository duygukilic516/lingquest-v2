...
function mergeLegacySession(target, legacy){
  ...
  target[id] = Object.assign({}, legacy, existing, {
    id,
    // keep v45 marker so current admin filter will display legacy + new together
    appVersion: existing.appVersion || legacy.appVersion || 'v45',
    statsNamespace: existing.statsNamespace || legacy.statsNamespace || 'events-v45',
  ...
  // Preserve important truthy fields from either side.
  ['listeningComplete','activitiesStarted','activitiesComplete','conversationStarted','conversationComplete','feedbackViewed','saveClicked'].forEach(k=>{
    target[id][k] = !!(existing[k] || legacy[k]);
  });

  if(existing.activitySummary || legacy.activitySummary) target[id].activitySummary = existing.activitySummary || legacy.activitySummary;
  if(existing.feedbackSummary || legacy.feedbackSummary) target[id].feedbackSummary = existing.feedbackSummary || legacy.feedbackSummary;
  if(existing.savedAt || legacy.savedAt) target[id].savedAt = existing.savedAt || legacy.savedAt;
  if(existing.completedAt || legacy.completedAt) target[id].completedAt = existing.completedAt || legacy.completedAt;
}
...
export default async function handler(req, res){
  ...
  try{
    ...
    // Read old pre-v45 session files too, so existing admin stats do not disappear after the append-only upgrade.
    ...
    legacySessions.forEach(s => mergeLegacySession(sessionMap, s));
